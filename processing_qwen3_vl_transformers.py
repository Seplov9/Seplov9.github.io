    ...
    def __call__(
        ...
        **kwargs: Unpack[Qwen3VLProcessorKwargs],
        ...
        if video_grid_thw is not None:
            merge_length = self.video_processor.merge_size**2  # 4
            index = 0
            for i in range(len(text)):
                while self.video_token in text[i]:
                    ...
                    '''
                    (Pdb) pp metadata
                    VideoMetadata(total_num_frames=5334,
                                  fps=29.97302764666217,
                                  width=1280,
                                  height=720,
                                  duration=177.96,
                                  video_backend='torchcodec',
                                  frames_indices=array([  ])  # len = 355
                                  )
                    '''
                    ...
                    video_placeholder = ""
                    frame_seqlen = video_grid_thw[index][1:].prod() // merge_length  # tensor(66)
                    for frame_idx in range(video_grid_thw[index][0]):  # video_grid_thw = tensor([[178,  12,  22]])
                    ...
                    '''
                    string.replace(old, new, 1) 告诉程序：只替换第一次遇到的匹配项，剩下的即使内容相同也不动。
                    在 while self.video_token in text[i]: 这个循环里，代码是在逐个处理视频。如果用户的 Prompt 里有三个视频标记（例如："视频A：<video>，视频B：<video>"），代码需要确保：
                    第一次循环：只把第一个 <video> 替换成第一个视频的帧和时间戳。
                    第二次循环：处理第二个 <video>。 
                    如果不加 1，第一次循环就会把文中所有的 <video> 都替换成“视频A”的内容，导致后面的视频信息全部错乱。
                    '''
                    else:
