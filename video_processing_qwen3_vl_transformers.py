...
class Qwen3VLVideoProcessor(BaseVideoProcessor):
    ...
    def _preprocess(
        ...
        **kwargs,
        ...
        # grouped_videos[0]: (355, 720, 1280) ： [1, 355, 3, 720, 1280] (shape)
        # grouped_videos_index = {0: ((355, 720, 1280), 0)}
        '''
        (Pdb) grouped_videos.keys()
        dict_keys([(355, 720, 1280)])
        (Pdb) pp list(grouped_videos.values())[0].shape
        torch.Size([1, 355, 3, 720, 1280])
        '''

        for shape, stacked_videos in grouped_videos.items():
            B, T, C, H, W = stacked_videos.shape  # [1, 355, 3, 720, 1280]
            ...
            if do_resize:
                ...
                stacked_videos = stacked_videos.view(B * T, C, H, W)  # [355, 3, 720, 1280]
                stacked_videos = self.resize(
                    stacked_videos,
                    size=SizeDict(height=resized_height, width=resized_width),
                    resample=resample,
                )  # [355, 3, 192, 352]
                stacked_videos = stacked_videos.view(B, T, C, resized_height, resized_width)  # [1, 355, 3, 192, 352]
        ...
        # grouped_videos[0]: (355, 192, 352) ： [1, 355, 3, 192, 352] (shape)
        # grouped_videos_index = {0: ((355, 192, 352), 0)}
        ...
        for shape, stacked_videos in grouped_videos.items():
            ...
            patches = stacked_videos  # [1, 355, 3, 192, 352]

            # Check that videos have `num_frames` divisible by `temporal_patch_size`
            T = patches.shape[1]
            if pad := -T % temporal_patch_size:
                repeats = patches[:, -1:].expand(-1, pad, -1, -1, -1)  # [1, 1, 3, 192, 352]
                patches = torch.cat((patches, repeats), dim=1)  # [1, 356, 3, 192, 352]
